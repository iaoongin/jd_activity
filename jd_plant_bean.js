...
async function jdPlantBean() {
  try {
    ...
    if ($.plantBeanIndexResult && $.plantBeanIndexResult.code === '0' && $.plantBeanIndexResult.data) {
      ...
      if ($.isNode() && message) {
        await notify.sendNotify(`${$.name}`, `${message}`, { url: `https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean` })
      }
    } else {
...
function requireConfig() {
  return new Promise(resolve => {
    ...
    // const jdPlantBeanShareCodes = $.isNode() ? require('./jd_plantBean_share_codes.js') : '';
