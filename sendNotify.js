...
// ======================================= server酱+企业微信（wecomchan） =======================================
//官方文档：https://console.cloud.tencent.com/scf/list-detail?rid=1&ns=default&id=wecomchan&menu=trigger&tab=codeTab
let WECOMCHAN_URL = '';
let WECOMCHAN_SEND_KEY = '';
...
if (process.env.WECOMCHAN_SEND_KEY) {
  WECOMCHAN_SEND_KEY = process.env.WECOMCHAN_SEND_KEY;
}
if (process.env.WECOMCHAN_URL) {
  WECOMCHAN_URL = process.env.WECOMCHAN_URL;
}
...
function wecomchanNotify(text, desp, timeout=2100) {
  return  new Promise(resolve => {
    if (WECOMCHAN_SEND_KEY) {
          ...
          try {
            if (err) {
              ...
              if (data.errno === 0 || data.data.errno === 0 ) {
                console.log('wecomchan发送通知消息成功\n')
              } else if (data.errno === 1024) {
                // 一分钟内发送相同的内容会触发
                console.log(`wecomchan发送通知消息异常: ${data.errmsg}\n`)
              } else {
                console.log(`wecomchan发送通知消息异常\n${JSON.stringify(data)}`)
              }
            }
          } catch (e) {
            $.logErr(e, resp);
      ...
      console.log('您未提供wecomchan的SEND，取消微信推送消息通知\n');
      resolve()
    }
  })
}
